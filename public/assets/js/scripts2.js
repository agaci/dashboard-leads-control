...
// Ação de clique
// Event Listeners para os botões do Step 2
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (btnConfirmar) {
        btnConfirmar.addEventListener('click', function() {
            ...
            // Ir para Step 4 (formulário de contacto)
            showStep(4);
        });
    }
    
    // Botão Prefiro Falar
    const btnFalar = document.getElementById('btnPrefiroFalar');
    if (btnFalar) {
        btnFalar.addEventListener('click', function() {
            console.log('💬 Prefiro Falar clicado');
            // Ir para Step 4 (formulário de contacto) mas com flag de callback
            formState.isCallback = true;
            showStep(4);
        });
    }
    
    // Link Alterar Endereços
    const btnChange = document.getElementById('btnChangeAddresses');
    if (btnChange) {
        btnChange.addEventListener('click', function(e) {
            e.preventDefault();
            console.log('← Voltar ao Step 1');
            showStep(1);
        });
    }
